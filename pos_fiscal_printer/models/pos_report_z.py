...
class PosReportZ(models.Model):
    ...
    nc_desde = fields.Char("Nota desde", tracking=True)
    nc_hasta = fields.Char("Nota hasta", tracking=True)
    ...
    def _compute_total_pos(self):
        ...
        for report in self:
            ...
            print('cantidad de ordenes de venta', len(report.pos_order_ids))
            for order in report.pos_order_ids:
                print('numero de factura', order.num_factura)
                if order.num_factura:
                    if order.num_factura not in fac_nums and int(order.num_factura) > 0:
                        fac_nums.append(int(order.num_factura))
            if fac_nums:
                if report.fac_desde == '' or report.fac_desde == '0' or not report.fac_desde:
                    report.fac_desde = min(fac_nums)
                if report.fac_hasta == '' or report.fac_hasta == '0' or not report.fac_hasta:
                    report.fac_hasta = max(fac_nums)


    def action_done(self):
        self._compute_total_pos()
        self.state = 'done'
