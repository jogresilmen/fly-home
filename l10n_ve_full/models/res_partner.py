...
class ResPartner(models.Model):
    ...
    municipality_id = fields.Many2one('res.country.municipality', 'Municipio', required=True, help="In this field enter the name of the associated municipality\n")
...
class CountryState(models.Model):
    ...
    municipalities_ids = fields.One2many('res.country.municipality', 'state_id', string='Municipalities', required=True, help="In this field enter the name of the associated municipalities to the state")
...
class MunicipalityParish(models.Model): 
    ...
    def _compute_display_name(self):
        for record in self:
            record.display_name = f"{record.name} ({record.municipality_id.code})"
...
class ResCountryMunicipality(models.Model):
    ...
    name = fields.Char('Municipio Nombre', size=64, required=True, help="En este campo ingrese el nombre del Municipio\n")
    code = fields.Char('Municipio Codigo', size=3, required=True, help='El código del municipio en tres números, Ejemplo: 001 para Libertador.')
    state_id = fields.Many2one('res.country.state', 'State', required=True, help="En este campo ingrese el nombre del estado al que está asociado el municipio.")
    parish_ids = fields.One2many('res.country.state.municipality.parish', 'municipality_id', string='Paroquias', required=True, help="En este campo ingrese el nombre de las parroquias del municipio \n")
