...
class TxtIva(models.Model):
    ...
    def get_document_affected(self, txt_line):
        ...
        """
        number = '0'
        if txt_line.invoice_id.move_type in ['out_refund', 'in_refund'] and txt_line.invoice_id.invoice_reverse_purchase_id:
            number = txt_line.invoice_id.invoice_reverse_purchase_id.supplier_invoice_number
    ...
    def get_amount_exempt_document(self, txt_line):
        ...
        _logger.info("GET_AMOUNT_EXEMPT_DOCUMENT")
        _logger.info(amount_doc)
        ...
        for txt in self:
            ...
            for txt_line in txt.txt_ids:
                ...
                if txt_line.voucher_id == txt_line.invoice_id.wh_iva_id:
                    ...
                    amount_total2 = round(amount_total11, 2)
                    amount_untaxed = txt_line.untaxed
                else:
                    amount_total2 = round(amount_total, 2)
                    amount_untaxed = amount_untaxed

                _logger.info("amount_untaxed2")
                _logger.info(amount_untaxed)
                untaxed2 = round(txt_line.untaxed, 2)
                amount_withheld2 = round(txt_line.amount_withheld, 2)
                amount_exempt2 = round(txt_line.amount_sdcf, 2)
                alicuota2 = alicuota
                if document_type == '05':
                ...
                _logger.info("amount_untaxed3")
                _logger.info(round(abs(txt_line.untaxed)))

                _logger.info("amount_untaxed4")
                _logger.info(self.formato_cifras(round(abs(txt_line.untaxed))))

                if float(amount_withheld2) < 0:
                    value1 = round(float(amount_withheld2) * -1, 2)
                else:
                    value1 = round(float(amount_withheld2), 2)
                if float(amount_untaxed) < 0:
                    value2 = round(float(amount_untaxed) * -1)
                else:
                    value2 = round(float(amount_untaxed), 2)
                ...
                txt_string = (
                        ...
                        self.formato_cifras(value2 if document_type != '03' else round(abs(txt_line.untaxed),2)) + '\t' +
