...
class AccountMove(models.Model):
    ...
    supplier_invoice_number = fields.Char(string='Supplier Invoice Number', store=True,
                            help="The reference of this invoice as provided by the supplier.", compute='get_sequence_supplier')
    invoice_datetime = fields.Datetime(string="Fecha emision de factura", store=True, )
    is_out_invoice = fields.Boolean(compute='_compute_is_out_invoice', string='Is Out Invoice', store=True)
    nro_ctrl = fields.Char(string='Número de Control', size=32,
                            help="Número utilizado para gestionar facturas preimpresas, por ley es necesario poner aquí este"
                                " número para poder declarar Informes fiscales correctamente.", copy=False, store=True,
                           compute='get_sequence')
    ...
    people_type_company1 = fields.Selection([('pjdo', 'PJDO Persona Jurídica Domiciliada'),
                                            ...
                                            string='Tipo de Persona compañía val', related='partner_id.people_type_company',store=True)
    people_type_individual1 = fields.Selection([('pnre', 'PNRE Persona Natural Residente'),
                                                ('pnnr', 'PNNR Persona Natural No Residente')],
                                               string='Tipo de Persona individual val', related='partner_id.people_type_individual',store=True)
    ...
    #campo relacionado desde res.partner contribuyente_seniat =
    contribuyente_seniat = fields.Selection(related='partner_id.contribuyente_seniat', string='Contribuyente SENIAT')

    is_admin = fields.Boolean(compute='_compute_is_admin')
    total_tax_ret = fields.Float(related='wh_iva_id.total_tax_ret', string='Total Retención IVA')
    amount_total_ret = fields.Float(related='islr_wh_doc_id.amount_total_ret', string='Total Retención ISLR')

    total_islr = fields.Monetary(string='Total Retención ISLR', compute='_compute_total_islr', store=True)
    total_iva = fields.Monetary(string='Total Retención IVA', compute='_compute_total_iva', store=True)

    def unlink(self):
        if self.nro_ctrl or self.supplier_invoice_number:
            raise AccessError('No se puede eliminar una factura que ya tenga asignada un número de control')
    ...
    def _compute_is_out_invoice(self):
        for rec in self:
            rec.is_out_invoice = rec.move_type == 'out_invoice' or rec.move_type == 'out_refund'
            # _logger.info('is_out_invoice: %s', rec.is_out_invoice)

    @api.depends('move_type')
    def get_sequence(self):
        for rec in self:
            if (rec.move_type == 'out_invoice' or rec.move_type == 'out_refund') and (rec.state == 'posted'):
                self.nro_ctrl = self.env['ir.sequence'].next_by_code('l10n_nro_control_sale')

    @api.depends('move_type')
    def get_sequence_supplier(self):
        for rec in self:
            if (rec.move_type == 'out_invoice' or rec.move_type == 'out_refund') and (rec.state == 'posted'):
                self.supplier_invoice_number = self.env['ir.sequence'].next_by_code('l10n_nro_supplier')
    @api.depends('total_tax_ret')

    def _compute_total_iva(self):
        for record in self:
            record.total_iva = record.total_tax_ret

    @api.depends('amount_total_ret')
    def _compute_total_islr(self):
        for record in self:
            record.total_islr = record.amount_total_ret
    ...
    def action_post(self):
        ...
        for res in self:
            ...
            # Convertir a la zona horaria de Caracas y eliminar la información de la zona horaria
            caracas_time = now_utc.astimezone(timezone('America/Caracas')).replace(tzinfo=None)
            if (res.move_type == 'out_invoice' or res.move_type == 'out_refund'):
                res.nro_ctrl = self.env['ir.sequence'].next_by_code('l10n_nro_control_sale')
                res.supplier_invoice_number = self.env['ir.sequence'].next_by_code('l10n_nro_supplier')

            # Asignar la hora a tu campo
            res.invoice_datetime = caracas_time
            ...
            if not res.journal_id.eliminar_impuestos:
                ...
                if res.company_id.partner_id.islr_withholding_agent and res.partner_id.islr_withholding_agent:
                    if res.state == 'posted' and not res.islr_wh_doc_id and len(res.invoice_line_ids.filtered(lambda x: x.concept_id.withholdable)) > 0:
        ...
        for r in res:
            if r.invoice_date and r.date:
                if r.invoice_date > r.date:
                    raise UserError(_('La fecha contable no puede ser menor a la fecha de la factura'))
        return res

    def write(self, vals):
        if vals.get('state'):
            # We need to prevent posted invoices to be reseted to draft
            # because of control number its already assigned, so, we need
            # to cancel it or make a credit note
            if ( self.state == 'posted' and vals['state'] == 'draft' ) and ( self.move_type in ['out_invoice', 'out_refund', 'out_receipt', 'in_invoice', 'in_refund', 'in_receipt'] ):
                raise AccessError('No se puede cambiar a borrador una factura que ya tiene un número de control asociado. Anule la factura o genere la correspondinte nota de crédito.')
            elif ( self.state == 'cancel' ) and ( self.move_type in ['out_invoice', 'out_refund', 'out_receipt', 'in_invoice', 'in_refund', 'in_receipt'] ):
                raise AccessError('No se puede modificar una factura que ya ha sido cancelada y tiene un numero de control asociado. Cree una nuea factura o genere la correspondinte nota de débito.')
            else: 
               pass 

        for move_account in self:
            if move_account.move_type == 'in_invoice' and move_account.invoice_origin:
                order_purchase = self.env['purchase.order'].search([('name', '=', self.invoice_origin),('company_id','=',self.company_id.id)], limit=1)
                if order_purchase:
    ...
    def ret_and_reconcile(self, pay_amount, pay_account_id,
        ...
        if module_dual_currency:
            if results is not None and 'partials' in results:
    ...
    def wh_iva_line_create(self):
        ...
        for inv_brw in self:
            ...
            if inv_brw.move_type in ('out_invoice', 'out_refund'):
                ...
                wh_iva_rate = partner._find_accounting_partner(inv_brw.company_id.partner_id).wh_iva_rate
            ...
            if rec.move_type in ('in_invoice', 'in_refund', 'in_receipt', 'out_receipt', 'out_refund', 'out_invoice'):
                ...
                for det_tax in lista_impuesto:
                    ...
                    if det_tax.type_tax == 'iva':
                        det_lin = rec.invoice_line_ids.filtered(lambda line: line.tax_ids.id == det_tax.id)
                        if det_lin:
                            for det_fac in det_lin:  # USAR AQUI ACOMULADORES
