...
class AccountMoveLine(models.Model):
    ...
    concept_id = fields.Many2one('account.wh.islr.concept', 'Concepto de Islr', ondelete='cascade',
                                ...
                                compute='_onchange_product_id_concept_id', store=True, readonly=False, precompute=True,)
    ...
    @api.depends('product_id','concept_id')
    def _onchange_product_id_concept_id(self):
        for line in self:
            if line.product_id.concept_id:
                line.concept_id = line.product_id.concept_id.id
            else:
                line.concept_id = self.env['account.wh.islr.concept'].search([('name', '=', 'NO APLICA RETENCION')]).id
