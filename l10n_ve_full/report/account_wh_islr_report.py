...
class RepComprobanteIslr(models.AbstractModel):
    ...
    def _get_report_values(self, docids, data=None):
        ...
        if date_ret:
            ...
            raise UserError(_("Se necesita la Fecha para poder procesar."))
        ...
        if data['form'].state == 'done':
            if data['form'].invoice_ids.invoice_id.currency_id.id == data['form'].company_id.currency_id.id:
            ...
            elif data['form'].invoice_ids.invoice_id.currency_id != data['form'].company_id.currency_id:
                ...
                if module_dual_currency:
                    total_doc = data['form'].invoice_ids.invoice_id.amount_total * data['form'].invoice_ids.invoice_id.tax_today
                else:
                    tasa = data['form'].invoice_ids.invoice_id.currency_id.inverse_rate
    ...
    def get_period(self, date):
        if not date:
            raise UserError(_("Se necesita una fecha, por favor ingresar"))
